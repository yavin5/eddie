...
/**
 * Object returned from loadPlugins(): the per-method callable surface plus the
 * metadata the bot reads (tools, llmFunctionNames). Modeled here rather than in
 * bot.ts to keep the loader self-contained.
 */
export interface LoadedPlugins {
  /** OpenAI-compatible tool schemas, one entry per @llmFunction method. Empty
   * until the loader populates it. */
  tools: { type: string; function: FunctionInfo }[];
  /** Names of the exposed LLM-callable plugin methods, in load order. */
  llmFunctionNames: string[];
  /** Any plugin method name maps to its callable function. */
...
export class PluginLoader {
   ...
   * all plugin functions and members, along with LLM-compatible tools metadata.
    * @returns {Promise<LoadedPlugins>} An object containing plugin functions, members, tools array, and LLM function names.
    */
   async loadPlugins(): Promise<LoadedPlugins> {
     const plugins: LoadedPlugins = { tools: [], llmFunctionNames: [] };
    let files: string[];
    try {
